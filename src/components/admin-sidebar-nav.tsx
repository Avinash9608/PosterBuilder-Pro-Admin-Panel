...
export function AdminSidebarNav() {
  ...
  return (
    <Sidebar collapsible="icon" variant="sidebar" side="left">
      <SidebarHeader className="h-16 flex items-center justify-between p-4">
        <Link href="/dashboard" className="flex items-center gap-2">
          <Button
            variant="ghost"
            className={cn(
              "h-10 w-10 p-0 text-primary",
              open ? "block" : "hidden group-data-[collapsible=icon]:block"
            )}
          >
            <Building size={28} />
          </Button>
          <h1
            className={cn(
              "text-xl font-semibold font-headline text-primary",
              open ? "block" : "hidden"
            )}
          >
            PosterBuilder
        ...
        <div
          className={cn(
            open ? "block" : "hidden group-data-[collapsible=icon]:hidden"
          )}
        >
          <SidebarTrigger />
        </div>
      ...
      <SidebarFooter
        className={cn(open ? "p-4" : "p-2 group-data-[collapsible=icon]:p-2")}
      >
